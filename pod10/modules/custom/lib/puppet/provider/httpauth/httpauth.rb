## resource[:file] --refers to value stored in file attr. of the httpauth resource
...
Puppet::Type.type(:httpauth).provide(:httpauth) do
  ...
  def create
    ...
    # flush method --flushes the buffer and writes our changes
    @htauth.flush
  end
  ...
  def check_passwd(realm, user, password, cp)
    if resource[:mechanism] == :digest
      WEBrick::HTTPAuth::DigestAuth.makepasswd(realm, user, password) == cp
    elsif resource[:mechanism] == :basic
      # Can't ask WEBrick as it uses a random seed
